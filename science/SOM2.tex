...
The data are collected on a Leica SP5 confocal microscope, using 532 nm laser excitation. The scanning volume is 98 $\times$ 98 $\times$ 53 $\mu$m$^3$, which contains $\sim 10^4$ colloid particles. Our spatial resolution is 192 nm / pixel. The Brownian time is $\tau_B \approx 4.5$ s. To be able to follow individual trajectories, we perform a 3D scan every 10 s at early time and every 30 s later.
