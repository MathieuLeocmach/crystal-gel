...
of the polymer concentration, $c_p=0.38,0.48,0.57,1.07$ mg/g for $\phi=0.25$ and $c_p=0.82,1.36$ mg/g for $\phi=0.10$.
In Fig. 1A, we superimpose the state points with the phase diagram calculated from the equation of state.
...
But a difference
in the fractal dimension $D$ between the $\phi=0.25$ and $c_p=0.38$ mg/g state point and other state points starts to become visible. In fact, while states
...
the state point with low polymer concentration ($\phi=0.25$ and $c_p=0.38$ mg/g) displays the biggest deviation from that exponent. In the figure we also
...
the structure factors (Fig.~\ref{fig:phase_separation}C), where the diffraction planes appear as sharper peaks for $\phi=0.25$ and $c_p=0.38$ mg/g.
...
at $\phi=0.25$ and $c_p=0.38$ mg/g reaches close packing at a volume fraction of around $\sim 60\%$, which is in reasonable agreement with the
...
This result clearly indicates that the state point at $\phi=0.25$ and $c_p=0.38$ mg/g follows a different arrest mechanism, in which phase separation
...
To confirm the formation of crystalline seeds we adopt bond orientational analysis to detect which particles are in crystalline environments~\cite{russo2013interplay}. With this analysis we confirm that crystallization events start occurring for the state point $\phi=0.25$ and $c_p=0.38$ mg/g, which has the lowest
...
$\phi=0.25$ and $c_p=0.38$ mg/g. The results show that the crystal growth follows two different scaling laws: at small crystalline sizes it scales with
...
First we show in Fig.~\ref{fig:transitions}A slabs of 10 particle diameter thickness for configurations at $\phi=0.25$ and $c_p=0.38$ mg/g at different times.
...
$\phi=0.25$ and $c_p=0.38$ mg/g, after the liquid-gas phase separation has occurred. The process of crystallization is characterized
...
(i) Crystallization, observed at $\phi=0.25$ and $c_p=0.38$ mg/g, small crystalline nuclei appear inside the liquid, reach the surface
...
\begin{figure}[!t]
 ...
 % crystal_size.eps: 0x0 pixel, 300dpi, 0.00x0.00 cm, bb=(atend)
\caption{Characterization of the crystal-gel. (A) Time evolution of the average size of the crystals (blue line) and the number of crystals (red line) for the state point $\phi=0.25$ and $c_p=0.38$ mg/g. (B) Radius of gyration of crystalline nuclei at the late stages of gelation for the same state point.} 
 \label{fig:crystals}
\end{figure}
...
(A) Reconstructions from confocal coordinates at $\phi=0.25$ and $c_p=0.38$ mg/g. Depth of view is $5\sigma$. Particles are drawn to scale and colored according to their phase. Dark red: crystal; orange: surface of crystal; yellow: liquid; blue: gas. 
